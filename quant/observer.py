...
class ObserverBase(nn.Module):
    ...
    def calculate_qparams(self, min_val, max_val):
        ...
        # ensure min_val and max_val are within quant range
        min_val_neg = torch.clamp(min_val_neg, -1e6, 1e6)
        max_val_pos = torch.clamp(max_val_pos, -1e6, 1e6)
        ...
        if self.symmetric:
            max_abs = torch.max(-min_val_neg, max_val_pos)
            ...
            scale = torch.max(scale, self.eps)
            initial_zero_point = quant_min - min_val_neg / scale
            zero_point = torch.round(initial_zero_point)

        # clamp scale and zero_point
        zero_point = torch.clamp(zero_point, quant_min, quant_max)
        scale = torch.clamp(scale, self.eps, 1.0)
...
class MinMaxObserver(ObserverBase):
    ...
    def forward(self, x_orig):
        ...
        # add epsilon to avoid zero scale
        self.min_val = torch.min(self.min_val, self.max_val - self.eps)
...
class MSEFastObserver(ObserverBase):
    ...
    def golden_section_search_2D_channel(self, x, x_min, x_max):
        ...
        # Corrected to remove warning: clone().detach() used instead of torch.tensor()
        best_min_tensor = torch.tensor(best_min).clone().detach().cuda()
        best_max_tensor = torch.tensor(best_max).clone().detach().cuda()

        return best_min_tensor, best_max_tensor
