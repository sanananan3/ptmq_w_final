...
from quant.quant_func import (
    ...
    fake_quantize_learnable_per_tensor_affine_train,
    fake_quantize_learnable_per_channel_affine_train,
...
class LSQFakeQuantize(QuantizeBase):
    ...
    def __init__(
        self, observer, bit=8, symmetric=False, ch_axis=-1, use_grad_scaling=True
    ):
        super().__init__(observer, bit=bit, symmetric=symmetric, ch_axis=ch_axis)
        ...
        if self.fake_quant_enabled == 1:
            ...
            if self.ch_axis != -1:
                if self.use_grad_scaling:
                    ...
                    grad_factor = 1.0
                X = fake_quantize_learnable_per_channel_affine_train(
                    X,
                    self.scale,
                    self.zero_point.data.int(),
                    self.ch_axis,
        ...
        if self.fake_quant_enabled == 1:
            ...
            if self.ch_axis != -1:
                if self.use_grad_scaling:
                    ...
                    grad_factor = 1.0
                X = fake_quantize_learnableplus_per_channel_affine_train(
                    X,
                    self.scale,
                    self.zero_point,
                    self.ch_axis,
